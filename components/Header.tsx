'use client';
import { useBoardStore } from '@/store/BoardStore';
import {useEffect, useState} from 'react' ;
import { MagnifyingGlassIcon, UserCircleIcon } from '@heroicons/react/20/solid'
import Image from 'next/image'
import React from 'react'
import Avatar from 'react-avatar'
import fetchSuggestion, { formatTodosForAI } from '@/lib/fetchSuggestion';

const Header = () => {
  const [board, searchString, setSearchString] = useBoardStore((state)=>[state.board, state.searchString, state.setSearchString]) ;
  const [loading, setLoading] = useState<boolean>(false)
  const [suggestion, setSuggestion] = useState<string>("") ;

   useEffect(()=>{
    if(board.columns.size===0) return ;
    
    setLoading(true) ; 
    // console.log(board);
    // debugger;
    // const todos = formatTodosForAI(board);
    // console.log(todos)

    const fetchSuggestionFunc = ()=>{
      
      const suggestion = fetchSuggestion(board) ;  
      setSuggestion(suggestion);
      setLoading(false) ;
    
    }

    const timeoutId = setTimeout(() => {
      fetchSuggestionFunc() ;
    }, 2000);

    // Cleanup the timeout if the component is unmounted
    return () => clearTimeout(timeoutId);

    
  }, [board]);


  return (
    <header className='w-full'>
    <div className='flex flex-col md:flex-row items-center p-5 bg-gray-500/10 justify-between w-full rounded-b-2xl'>
     <div className="
                    absolute
                    top-0
                    left-0
                    w-full
                    h-96
                    bg-gradient-to-br
                  from-pink-400
                   to-[#0055d1]
                   rounded-md
                   filter
                   blur-3xl
                   opacity-50
                   -z-50
     " />
      <Image 
      src="https://links.papareact.com/c2cdd5" 
      alt='Trello log'
      width={300}
      height={100}
      className='w-44 md:w-56 pb-10 md:pb-0'
      />
      <div className='flex items-center space-x-5 flex-1 justify-end' >
        {/* Search box  */}
        <form className='flex items-center space-x-5 bg-white rounded-md p-2 shadow-md'>
            <MagnifyingGlassIcon className='h-6 w-6 text-gray-400'/> 
            <input type='text' 
            value={searchString}
            onChange={(e)=>setSearchString(e.target.value)}
            className='flex-1 outline-none p-2 text-black'/>
            <button hidden>Search</button>
        </form>

        {/* Avatar */}
        <Avatar name='Rohit Renke' round color='#0055D1' size='50'/>

      </div>
      </div>
      <div className='flex items-center justify-center py-3 px-5 md:py-5'>
        <p className='flex items-center text-sm font-light pr-5 shadow-xl rounded-xl w-fit bg-white italic max-w-3xl text-[#0055D1] p-5'>
          <UserCircleIcon className={`inline-block h-10 w-10 text-[#0055D1] ${loading && 'animate-spin'}`}/>
          {
            suggestion && !loading? suggestion: 'GPT is summarising your tasks for the day...'
          }
        </p>
      </div>
    </header>
  )
}

export default Header
