...
const Column = ({id, todos, index}:Props) => {
 ...
 const handleAddTodo =()=>{
    ...
    return (
        ...
        >
            {/* render droppable todod in the column */}
            <Droppable droppableId={index.toString()} type='card'>
                {
                    ...
                    >
                        <h2 className='flex justify-between font-bold text-xl p-2'>
                            {idToColumnText[id]} 
                            <span className='text-gray-500 bg-gray-200 rounded-full px-2 py-1 text-sm'>{!searchString? todos.length: todos.filter(todo=>!!todo.title.toLowerCase().includes(searchString.toLowerCase())).length}</span>
                        </h2>
